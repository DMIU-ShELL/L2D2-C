# DeepRL for single task and multi-task lifelong (continual learning)
This repository is a fork and modification of the [DeepRL](https://github.com/shangtongzhang/deeprl) library.

Currently, the primary environments for experiments are:
* [CT-graph](https://github.com/soltoggio/CT-graph)
* [DynamicGrid](https://github.com/DMIU-ShELL/dynamic-grid)

## Installation

## Basic experiments
To run a single task (standard deeprl) experiment, run the command below. The script contains PPO, A2C and DQN agents setup for evaluation in CT-graph and DynamicGrid environments.
`python example_st.py`

To run a continual lifelong learning (for multiple task) experiment, run the command below. The script contains PPO, A2C and DQN agents setup for evaluation in CT-graph and DynamicGrid environments. In addition, each agent can be set up to synaptic consolidation methods (EWC, MAS or SCP).
`python example_cl.py`
