    ...
    for subdir, dirs, files in os.walk(root_directory):
        for file in files:
            if file == 'distances.csv':
                ...
                process_data(os.path.join(subdir, file), 'Distances')
            elif file == 'maha_cov_mean.csv':
                # Process covariance data for mean and generate graph
                process_data(os.path.join(subdir, file), 'Mahalanobis Covariance (Mean)')
            elif file == 'maha_cov_ident.csv':
                # Process covariance data for identity and generate graph
                process_data(os.path.join(subdir, file), 'Mahalanobis Covariance (Identity)')
            elif file == 'cos_sim.csv':
                # Process covariance data for identity and generate graph
                process_data(os.path.join(subdir, file), 'Cosine Similarity')
            elif file == 'density.csv':
                # Process covariance data for identity and generate graph
                process_data(os.path.join(subdir, file), 'Density')
            elif file == 'emd.csv':
                # Process covariance data for identity and generate graph
                process_data(os.path.join(subdir, file), 'Wasserstein Distance')
            elif file == 'wdist_log.csv':
                # Process covariance data for identity and generate graph
                process_data(os.path.join(subdir, file), 'Wasserstein Distance from Reference')