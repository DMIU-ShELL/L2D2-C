...
class ParallelComm(object):
        ...
        if expecting:
            ...
            for idx in expecting:
                ...
                # If not rejected then we need to send the mask to the requester
                    ...
                    d = {}
                    d['requester_agent_id'] = int(buff[ParallelComm.META_INF_IDX_PROC_ID])
                    d['msg_type'] = int(buff[ParallelComm.META_INF_IDX_MSG_TYPE])
                    d['msg_data'] = int(buff[ParallelComm.META_INF_IDX_MSG_DATA])
                    d['task_label'] = buff[ParallelComm.META_INF_IDX_TASK_SZ : ]
                    ret.append(d)
    ...
    '''
    def communication(self, msg, track_tasks, mask_rewards_dict, expecting, best_agent_id, await_response, queue_comm, queue_agent):
        while True:
            try:
                # Wait to receive a tasklabel from the agent to query the network
                # if not already been given at the start
                if msg is not None:
                    msg = queue_comm.get_nowait()
                
                if self.mode == 'ondemand':
                    # send out broadcast request to all other agents for task label
                    other_agents_request = []
    ...
    def parallel(self, msg, track_tasks, mask_rewards_dict, expecting, best_agent_id, await_response, queue_comm, queue_agent):
        p = mp.Process(target=self.communication, args=(msg, track_tasks, mask_rewards_dict, expecting, best_agent_id, await_response, queue_comm, queue_agent))
        p.start()
        return p
        p.join()