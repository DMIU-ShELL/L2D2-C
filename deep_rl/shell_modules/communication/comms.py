...
class ParallelComm(object):
    ...
    # Client used by the server to send queries and responses
    ...
    def send_query(self, dict_to_query):
        ...
        reward = dict_to_query.get('reward', default = 0.0)
    ...
    def recv_mask(self, buffer):
        ...
        Returns:
            received_mask: A torch tensor containing the continuous mask parameters.
            received_label: A torch tensor containing the embedding.
            received_reward: The reward associated with the received data.
            ip: The IP address from which the data was received.
            port: The port from which the data was received.
        """
        
        received_mask, received_label, received_reward, ip, port = None, None, None, None, None

        msg_data = buffer[ParallelComm.META_INF_IDX_MSG_DATA]

        if msg_data == ParallelComm.MSG_DATA_NULL:
            pass
        elif msg_data == ParallelComm.MSG_DATA_MSK:
    ...
    def recv_query(self, buffer):
        ...
        """
        sender_address = str(buffer[ParallelComm.META_INF_IDX_ADDRESS])
        sender_port = int(buffer[ParallelComm.META_INF_IDX_PORT])
        embedding = buffer[ParallelComm.META_INF_IDX_TASK_SZ]
        ...
        if (sender_address, sender_port) not in self.query_list:
            self.client([self.init_address, self.init_port, ParallelComm.MSG_TYPE_SEND_TABLE, list(self.query_list)], sender_address, sender_port)
    ...
    def send_meta(self, meta_resp):
        if meta_resp and meta_resp['response']:
            data = [
                self.init_address,
                self.init_port,
                ParallelComm.MSG_TYPE_SEND_META,
                ParallelComm.MSG_DATA_META,
    ...
    def proc_mask(self, mask_req, queue_label_send, queue_mask_recv):
        ...
        return mask_req  # Return the original dictionary if there's no response
    ...
    def send_mask(self, mask_resp):
        ...
        """
        if mask_resp and mask_resp['response']:
            data = [
                self.init_address,
                self.init_port,
                ParallelComm.MSG_TYPE_SEND_MASK,
                ParallelComm.MSG_DATA_MSK,
                mask_resp.get('mask', None),
                mask_resp.get('embedding', None),
                mask_resp.get('reward', None)
            ]
    ...
    def query(self, data):
        ...
        # Process the query and send a response if necessary
        meta_resp = self.proc_meta(other_agent_req)
        self.logger.info(f'Processed mask request: {meta_resp}')

        self.send_meta(meta_resp)
    def add_meta(self, data):
        """
        Add received metadata to the global list and print it.
        
        Args:
            data: A list containing metadata information.
        """
        metadata_entry = {
            'address': data[0],
            'port': data[1],
        ...
        print(f'Address: {metadata_entry["address"]}')
        print(f'Port: {metadata_entry["port"]}')
        print(f'Reward: {metadata_entry["reward"]}')
        print(f'Distance: {metadata_entry["dist"]}')
        print(f'Embedding: {metadata_entry["embedding"]}')
    def pick_meta(self):
        data = [self.init_address, self.init_port, ParallelComm.MSG_TYPE_SEND_REQ]

        # Time to pick the best agent
        if len(self.metadata) > 0:
            meta_copy = sorted(self.metadata, key=lambda d: (d['dist'], -d['reward']))  # bi-directional multikey sorting using distance and reward
            ...
            for meta_dict in meta_copy:
                ...
                self.logger.info(recv_rw, recv_dist, recv_emb)
                
                if recv_rw == torch.inf:
                    continue

                if recv_dist <= self.threshold and tuple(recv_emb) not in self.knowledge_base.keys():
                    data.append(ParallelComm.MSG_DATA_MSK_REQ)
                    data.append(recv_emb)
                    self.client(data, meta_dict['address'], meta_dict['port'])
                    break
