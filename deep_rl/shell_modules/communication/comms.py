...
class ParallelCommEval(object):
    ...
    # Task label size can be replaced with the embedding size.
    def __init__(self, num_agents, embd_dim, mask_dim, logger, init_port, mode, reference, knowledge_base, manager, localhost):
        super(ParallelCommV1Eval, self).__init__()
        self.embd_dim = embd_dim            # Dimensions of the the embedding
        self.mask_dim = mask_dim            # Dimensions of the mask for use in buffers. May no longer be needed
        self.logger = logger                # Logger object for logging CLI outputs.
        self.mode = mode                    # Communication operation mode. Currently only ondemand knowledge is implemented
    ...
    def _null_message(self, msg):
        ...
        # check whether message sent denotes or is none.
        if bool(msg[ParallelCommV1Eval.META_INF_IDX_MSG_DATA] == ParallelCommV1Eval.MSG_DATA_NULL):
            return True
    ...
    def send_join_net(self, ):
        ...
        data = [self.init_address, self.init_port, ParallelCommV1Eval.MSG_TYPE_SEND_JOIN]
    ...
    def recv_join_net(self, data):
        ...
        Args:
            ...
            world_size: A shared memory variable of type int() used to keep track of the size of the fully connected network.
        """
        address = data[ParallelCommV1Eval.META_INF_IDX_ADDRESS]        # new peer address
        port = data[ParallelCommV1Eval.META_INF_IDX_PORT]              # new peer port

        # Send this agent's query table and world size to the new agent.
        response = [self.init_address, self.init_port, ParallelCommV1Eval.MSG_TYPE_SEND_TABLE, list(self.query_list)]
        self.client(response, address, port)
    ...
    def send_exit_net(self):
        ...
        data = [self.init_address, self.init_port, ParallelCommV1Eval.MSG_TYPE_SEND_LEAVE]
    ...
    def send_query(self, embedding):
        ...
        if embedding is None:
            data = [self.init_address, self.init_port, ParallelCommV1Eval.MSG_TYPE_SEND_QUERY, ParallelCommV1Eval.MSG_DATA_NULL]

        else:
            data = [self.init_address, self.init_port, ParallelCommV1Eval.MSG_TYPE_SEND_QUERY, ParallelCommV1Eval.MSG_DATA_QUERY, np.array(embedding)]
    ...
    def recv_query(self, buffer):
        ...
        ret = {}
        ret['sender_address'] = str(buffer[ParallelCommV1Eval.META_INF_IDX_ADDRESS])
        ret['sender_port'] = int(buffer[ParallelCommV1Eval.META_INF_IDX_PORT])
        ret['msg_type'] = int(buffer[ParallelCommV1Eval.META_INF_IDX_MSG_TYPE])
        ret['msg_data'] = int(buffer[ParallelCommV1Eval.META_INF_IDX_MSG_DATA])
        ret['embedding'] = torch.tensor(buffer[ParallelCommV1Eval.META_INF_IDX_TASK_SZ])
    ...
    def send_meta(self, meta_response):
        ...
        data = [self.init_address, self.init_port, ParallelCommV1Eval.MSG_TYPE_SEND_META]
        ...
        if meta_response:
            ...
            if distance == torch.inf and mask_reward == torch.inf:
                data.append(ParallelCommV1Eval.MSG_DATA_NULL)
                data.append(torch.inf)
                data.append(torch.inf)
                data.append(torch.inf)
            
            else:
                data.append(ParallelCommV1Eval.MSG_DATA_META)
                data.append(mask_reward)
                data.append(distance)
                data.append(np.array(embedding))
    ...
    def recv_meta(self, buffer):
        ...
        ret = {}
        if self._null_message(buffer):
            ret['address'] = str(buffer[ParallelCommV1Eval.META_INF_IDX_ADDRESS])
            ret['port'] = int(buffer[ParallelCommV1Eval.META_INF_IDX_PORT])
            ret['mask_reward'] = 0.0
            ret['dist'] = torch.inf
            ret['embedding'] = None
        
        elif buffer[ParallelCommV1Eval.META_INF_IDX_MSG_DATA] == torch.inf:
            # Code should never reach this point. If it does then reality has broken.
            pass

        elif buffer[ParallelCommV1Eval.META_INF_IDX_MSG_DATA] == ParallelCommV1Eval.MSG_DATA_META:
            ret['address'] = str(buffer[ParallelCommV1Eval.META_INF_IDX_ADDRESS])
            ret['port'] = int(buffer[ParallelCommV1Eval.META_INF_IDX_PORT])
            ret['mask_reward'] = float(buffer[ParallelCommV1Eval.META_INF_IDX_MSK_RW])
            ret['dist'] = float(buffer[ParallelCommV1Eval.META_INF_IDX_DIST])
            ret['embedding'] = buffer[ParallelCommV1Eval.META_INF_IDX_TASK_SZ_]   # This is an np array
        ...
        if len(metadata) > 0:
            ...
            for key, data_dict in metadata.items():
                ...
                # Otherwise unpack the metadata
                    ...
                    # hasn't been selected yet then continue
                    if recv_msk_rw != 0.0:
                        if recv_dist <= ParallelCommV1Eval.THRESHOLD:
                            # Check if the reward is greater than the current reward for the task
                            # or if the knowledge even exists.
                            if tuple(recv_label) in self.knowledge_base.keys():
                                #self.logger.info(f'COMPARISON TAKES PLACE FOR CASE 1: {round(recv_msk_rw, 6)} > {0.75 * self.knowledge_base[tuple(recv_label)]}')
    ...
    def send_mask_req(self, send_msk_requests):
        ...
        if send_msk_requests:
            for data_dict in send_msk_requests:
                ...
                data = [self.init_address, self.init_port, ParallelCommV1Eval.MSG_TYPE_SEND_REQ]
                ...
                if embedding is None:
                    ...
                    # Otherwise we want the agent's mask
                    data.append(ParallelCommV1Eval.MSG_DATA_MSK_REQ)
                    data.append(np.array(embedding)) # NOTE deepcopy?
    ...
    def send_mask(self, mask_resp):
        ...
        if mask_resp:
            ...
            data = [self.init_address, self.init_port, ParallelCommV1Eval.MSG_TYPE_SEND_MASK]

            if mask is None or embedding is None:
                data.append(ParallelCommV1Eval.MSG_DATA_NULL)

            else:
                data.append(ParallelCommV1Eval.MSG_DATA_MSK)
                data.append(mask)
                data.append(embedding)
    ...
    def recv_mask(self, buffer, best_agent_id):
        ...
        if buffer[ParallelCommV1Eval.META_INF_IDX_MSG_DATA] == torch.inf:
            pass

        elif buffer[ParallelCommV1Eval.META_INF_IDX_MSG_DATA] == ParallelCommV1Eval.MSG_DATA_MSK:
            if {buffer[ParallelCommV1Eval.META_INF_IDX_ADDRESS]: buffer[ParallelCommV1Eval.META_INF_IDX_PORT]} == best_agent_id:
                received_mask = buffer[4]
                received_label = buffer[5]
    ...
    def update_params(self, data):
        ...
        self.query_list.extend(data[3])
        self.query_list.extend(temp)
        #self.query_list.append(Address(inet4=data[ParallelCommV1Eval.META_INF_IDX_ADDRESS], port=data[ParallelCommV1Eval.META_INF_IDX_PORT]))   # Append the reference agent ip-port
            ...
            with conn:
                ...
                while True:
                    try:
                        ...
                        ### EVENT HANDLING
                        # Agent is sending a query table
                        if data[ParallelCommV1Eval.META_INF_IDX_MSG_TYPE] == ParallelCommV1Eval.MSG_TYPE_SEND_TABLE:
                            t_table = mpd.Pool(processes=1)
                            t_table.apply_async(self.update_params, (data,))
                            t_table.close()
                            del t_table
                        ...
                        # An agent is sending a query
                        elif data[ParallelCommV1Eval.META_INF_IDX_MSG_TYPE] == ParallelCommV1Eval.MSG_TYPE_SEND_QUERY:
                            t_query = mpd.Pool(processes=1)
                            t_query.apply_async(self.query, (data, ))
                            t_query.close()
                            del t_query
                        ...
                        # An agent is sending some task distance and reward information in response to a query
                        elif data[ParallelCommV1Eval.META_INF_IDX_MSG_TYPE] == ParallelCommV1Eval.MSG_TYPE_SEND_META:
                            t_meta = mpd.Pool(processes=1)
                        ...
                        # An agent is sending a direct request for a mask
                        elif data[ParallelCommV1Eval.META_INF_IDX_MSG_TYPE] == ParallelCommV1Eval.MSG_TYPE_SEND_REQ:
                            t_req = mpd.Pool(processes=1)
                            t_req.apply_async(self.req, (data, queue_label_send, queue_mask_recv))
                            t_req.close()
                            del t_req
                        ...
                        # An agent is sending a mask
                        elif data[ParallelCommV1Eval.META_INF_IDX_MSG_TYPE] == ParallelCommV1Eval.MSG_TYPE_SEND_MASK:
                            t_msk = mpd.Pool(processes=1)
                            recv_mask, recv_embedding = t_msk.apply_async(self.recv_mask, (data, best_agent_id)).get()
                            t_msk.close()
                            del t_msk
        ...
        while True:
            ...
            try:
                ...
                try:
                    # Send out a query when shell iterations matches mask interval if the agent is working on a task
                    if self.world_size.value > 1:
                        if int(np.random.choice(2, 1, p=[ParallelCommV1Eval.DROPOUT, 1-ParallelCommV1Eval.DROPOUT])) == 1:  # Condition to simulate % communication loss
                            self.send_query(msg)
