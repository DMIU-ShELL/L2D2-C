...
class Communication(object):
    ...
    def _send_response(self, req_dict):
        #### TO DO: Swap masks with mask rewards and ID
    ...
    def broadcast_existence(self):
        '''
        Tell the network that a new agent has entered the network. Necessary information needs to be updated on 
        each agent in the network such as num agents in the network
        '''
        pass
...
class Communication_mp_old(object):
    META_INF_SZ = 3
    META_INF_IDX_PROC_ID = 0
    META_INF_IDX_MSG_TYPE = 1
    META_INF_IDX_MSG_DATA = 2
    ...
    # Wrapper for torch distributed irecv function
    # Handles the recieve response functionality in receive_response for multiprocessing
    def irecv_wrapper(self, queue_in, queue_out):
        if not queue.empty():
            tensor, src = queue_in.get()
            data = dist.irecv(tensor, src)
    ...
    def _send_response(self, queue_in):
        if not queue_in.empty():
            req_dict = queue_in.get()
        requester_agent_id = req_dict['sender_agent_id']
        mask = req_dict['mask']
        buff = self.buff_send_resp[requester_agent_id]
        buff[Communication.META_INF_IDX_PROC_ID] = self.agent_id
    ...
    def receive_response(self):
        ...
        for idx in range(self.num_agents):
            ...
            if self.handle_recv_resp[idx] is None:
                self.logger.info('recv_resp: set up handle to receive response from agent {0}'.format(idx))
                queue_in.put((self.buff_recv_resp[idx], idx))

                while not queue_out.empty():
                    self.handle_recv_resp[idx] = queue_out.get()
                #self.handle_recv_resp[idx] = dist.irecv(tensor=self.buff_recv_resp[idx], src=idx)
