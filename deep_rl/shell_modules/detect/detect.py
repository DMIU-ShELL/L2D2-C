...
class AttentionWassersteinEmbedding(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        super(AttentionWassersteinEmbedding, self).__init__()
        self.input_dim = input_dim
        ...
        # Define Wasserstein embedding layers
        self.embedding_layer = nn.Linear(input_dim, hidden_dim)
        self.output_layer = nn.Linear(hidden_dim, output_dim)

    def forward(self, x_ref, x_input):
        # Apply attention mechanism
        attention_weights = F.softmax(self.attention(x_input), dim=1)
        attended_input = torch.mul(x_input, attention_weights)
        
        # Compute Wasserstein distance between attended input and reference
        wasserstein_distance = self.compute_wasserstein_distance(attended_input, x_ref)
        
        # Apply Wasserstein embedding layers
        embedded_output = F.relu(self.embedding_layer(attended_input))
        output = self.output_layer(embedded_output)
        
        return output, wasserstein_distance

    def compute_wasserstein_distance(self, x1, x2):
        # Compute pairwise distance matrices
        C = torch.cdist(x1, x2, p=2)
        
        # Compute Wasserstein distance using OT library
        wasserstein_distance = torch.tensor([ot.emd2([], [], C.detach().cpu().numpy())])
        
        return wasserstein_distance

class AttentionDetect(Detect):
    def __init__(self, reference_num, input_dim, num_samples, reference=200, device='cpu', title='', num_iter=10, one_hot=True, normalized=True, demo=True):
        super().__init__(reference_num, input_dim, num_samples, reference, device, title, num_iter, one_hot, normalized, demo)
        # Define your attention mechanism or model here
        hidden_dim = 50
        output_dim = 30
        self.attention_model = AttentionWassersteinEmbedding(input_dim, hidden_dim, output_dim)

    def lwe(self, X, some_task_action_space_size):
        X = self.preprocess_dataset(X, some_task_action_space_size)
        # Apply attention mechanism to X to obtain attended features
        attended_features = self.attention_model(X)
        # Proceed with Wasserstein embedding calculation using attended features
        ref_size = self.ref.shape[0]
        C = ot.dist(attended_features.cpu(), self.ref).cpu().numpy()
        gamma = torch.from_numpy(ot.emd(ot.unif(attended_features.shape[0]), ot.unif(ref_size), C, numItermax=700000)).float()
        f = (torch.matmul((ref_size * gamma).T, attended_features.cpu()) - self.ref) / np.sqrt(ref_size)
        return f.ravel()
    
    def lwe(self, X, some_task_action_space_size):
        X = self.preprocess_dataset(X, some_task_action_space_size)
        output, wasserstein_distance = self.attention_model(self.ref, X)
        return output, wasserstein_distance
