...
def main(args):
    ...
    for i in range(len(xlabels)):
        ax.text(i, bar_x[i], str(bar_x[i]) + ' (' + str((round((bar_x[i]/_max_time_shell)*100, 2))) + '%)', ha = 'center', fontsize=15)
    for i in range(len(xlabels)):
    ...
    maximum_icr_ = 0.95 * maximum_icr_
    _max_time_shell = data['icr']['shell']['xdata'][np.where(data['icr']['shell']['ydata'] >= maximum_icr_)[0][0]]
    _max_time_ll = data['icr']['ll']['xdata'][np.where(data['icr']['ll']['ydata'] >= maximum_icr_)[0][0]]
    print(_max_time_shell, _max_time_ll)
    _seventy = 0.70 * maximum_icr_
    ...
    plt.ylabel('Evaluation checkpoints', fontsize=15)
    plt.xlabel("X% of 95% performance", fontsize=15)
    plt.legend(loc='upper left')
    ...
    if args.ll_paths is not None:
        ...
        tla = (((data['tpot']['shell']['ydata'])[0:len(data['tpot']['ll']['ydata'])] + 0.03125) / ((data['tpot']['ll']['ydata'])[0:len(data['tpot']['shell']['ydata'])] + 0.03125)) - 1
        data['tla']['shell'] = {}
        ...
        ila = (((data['icr']['shell']['ydata'])[0:len(data['icr']['ll']['ydata'])] + 0.03125) / ((data['icr']['ll']['ydata'])[0:len(data['icr']['shell']['ydata'])] + 0.03125)) - 1
        data['ila']['shell'] = {}
