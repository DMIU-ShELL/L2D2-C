...
def plot_tra(xdata, ydata, num_agents):
    ...
    ax.set_ylabel('TTp(SingleLLAgent) / TTp(C3L)', fontsize=14)
    ax.yaxis.grid(True)
    ax.xaxis.grid(True)
...
def cfi_delta(data, conf_int_param=0.75): # confidence interval
...
def main(args):
    ...
    mypaths = {
        'full comm' : ['Experiments/full_comm/seed1/MetaCTgraph-shell-dist-ct28_md_eval_full-seed-9183/agent_26/240209-150346/',
                        'Experiments/full_comm/seed2/MetaCTgraph-shell-dist-ct28_md_eval_full-seed-2745/agent_26/240210-124604/'],

        'no comm' : ['Experiments/no_comm/seed1/MetaCTgraph-shell-dist-ct28_md_eval_full-seed-9183/agent_26/240209-164032/',
                    'Experiments/no_comm/seed2/MetaCTgraph-shell-dist-ct28_md_eval_full-seed-2745/agent_26/240210-155928/']
    }
    ...
    for name, myarr in mypaths.items():
        ...
        # icr
        data['icr']['C3L ' + name] = {}
        data['icr']['C3L ' + name]['xdata'] = np.arange(num_evals)
        data['icr']['C3L ' + name]['ydata'] = np.mean(shell_icr, axis=0) # average across seeds
        data['icr']['C3L ' + name]['ydata_cfi'] = cfi_delta(shell_icr)
        data['icr']['C3L ' + name]['plot_colour'] = 'green'

        maximum_icr_ = math.ceil(max(maximum_icr_, np.max(data['icr']['C3L ' + name]['ydata'])))

        # tpot
        data['tpot']['C3L ' + name] = {}
        data['tpot']['C3L ' + name]['xdata'] = np.arange(num_evals)
        data['tpot']['C3L ' + name]['ydata'] = np.mean(shell_tpot, axis=0) # average across seeds
        data['tpot']['C3L ' + name]['ydata_cfi'] = cfi_delta(shell_tpot)
        data['tpot']['C3L ' + name]['plot_colour'] = 'green'
