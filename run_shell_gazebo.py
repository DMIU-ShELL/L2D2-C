...
from deep_rl.component.task import ParallelizedTask, MiniGridFlatObs, MetaCTgraphFlatObs, ContinualWorld, GazeboEnvWrapper
from deep_rl.network.network_heads import CategoricalActorCriticNet_SS, GaussianActorCriticNet_SS
from deep_rl.network.network_bodies import FCBody_SS, DummyBody_CL, GazeboConvBody_SS, GazeboConvBody_CL
import argparse
import torch
import random
import time
...
def global_config(config, name):
    ...
    config.use_gae = True
    config.gae_tau = 0.99
    config.entropy_weight = 0.1 #0.00015
    config.rollout_length = 128
    config.optimization_epochs = 1#8
    config.num_mini_batches = 64
    config.ppo_ratio_clip = 0.1
    config.iteration_log_interval = 1
    config.gradient_clip = 5
    config.max_steps = 25600
    config.evaluation_episodes = 0#50
...
def shell_dist_gazebo(name, args, shell_config):
    ...
    # set seed
    config.seed = 8379#config_seed              # Chris
...
def shell_dist_gazebo_eval(name, args, shell_config):
    shell_config_path = args.shell_config_path
    #num_agents = args.num_agents
...
if __name__ == '__main__':
    ...
    parser.add_argument('--shell_config_path', help='shell config', default='./shell_gazebo.json')                       # File path to your chosen shell.json configuration file. Changing the default here might save you some time.
    ...
    parser.add_argument('--localhost', '--ls', '-l', help='used to run DMIU in localhost mode', action='store_true')   # Flag used to start the system using localhost instead of public IP. Can be useful for debugging network related problems.
    ...
    if shell_config['env']['env_name'] == 'Gazebo':
        name = 'Gazebo'
        if args.eval:
            shell_dist_gazebo_eval(name, args, shell_config)

        else:
            shell_dist_gazebo(name, args, shell_config)
